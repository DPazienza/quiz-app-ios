const CACHE_NAME = 'quiz-app-ios-v4-app';
const TEST_CACHE_NAME = 'quiz-app-ios-test-packs-v1';
...
self.addEventListener('activate', event => {
  event.waitUntil(
    caches.keys().then(keys => Promise.all(
      keys.filter(k => k !== CACHE_NAME && k !== TEST_CACHE_NAME).map(k => caches.delete(k))
...
self.addEventListener('fetch', event => {
  ...
  if (url.pathname.includes('/tests/') || url.pathname.endsWith('.quiz.json')) {
    event.respondWith(
      caches.open(TEST_CACHE_NAME).then(cache =>
